...
var okLeft = false // ПЕРЕМЕННЫЕ ДЛЯ ОТСЛЕЖИВАНИЯ КЛАВИШ В ЛЕВО И ВПРАВО
var okRight = false
...
class Ball {
    ...
    drawBall() {
        ...
        if (this.y + 10 === 480 && this.x + 10 > myPlatf.x &&
            this.x - 10 < myPlatf.x + 100) {
            this.addY = -this.addY
        }
...
var render = function() { //Основная функция для отрисовки анимации
    if (stop === true) { return }
    drawRect()
...
addEventListener("keyup", function(event) { //keyup - єто отжатие клавиши
    var newDirect = event.keyCode
